...
/** Soft blue nodes on dark hero — matches pre-stripped creators reactive grid */
...
export function CreatorsPixelNetworkBackground() {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      for (const edge of edges) {
        ...
        const alpha =
          GLOBAL_ALPHA * Math.min(0.28, pulse + influence * 0.18);
      ...
      for (const node of nodes) {
        ...
        const alpha =
          GLOBAL_ALPHA * Math.min(0.38, glow + influence * 0.24);
      ...
      bottomFade.addColorStop(0, "rgba(23,23,23,0)");
      bottomFade.addColorStop(0.5, "rgba(23,23,23,0)");
      bottomFade.addColorStop(1, "rgba(23,23,23,0.8)");
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 z-0 h-full w-full pointer-events-none opacity-85"
      aria-hidden
    />
  );
}
